...
**Session 25 — Audio Pipeline Surgical Fix, Server-Side VAD, and Lazy Module Imports (2026-08-23)**: Resolved audio chopping and self-interruption by reverting `FRIDAY_VOICE_HEADPHONES_MODE` to `false` in `.env` and `.env.example` and re-enabling half-duplex echo suppression (`echo_mute=True`). Configured high-threshold local barge-in (`RMS 4000.0`) in the CLI so sustained shouting interrupts FRIDAY while background room noise is suppressed. Ensured continuous microphone PCM audio dispatch to Gemini Live whenever the speaker is idle. Enforced active microphone unmuting (`microphone.set_muted(False)`) when speaker is idle, added a 10s playback hard timeout to prevent deadlocks, and added console transition logging (`[DEBUG] Mic unmuted, listening...`). Optimized startup time by making heavy dependencies (`numpy`, `resemblyzer`, `pytesseract`, `PIL`) lazily imported inside functions across `voice_biometrics.py`, `screen_ocr.py`, `screen_snapshot.py`, and `detector.py`. Verified 100ms jitter prebuffer in `SpeakerStream` with clean, unpurged audio playback queues. **Verification**: `pytest -m "not live and not hardware" -q` passed (1011 passed, 4 skipped, 9 deselected in 155.96s).
